...
class ConversationListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if len(participants_data) != 2:
            return Response({
                'error':"A conversation exactly needs two participants"
...
class MessageListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return conversation.message.order_by('timestamp')
    ...
    def get_conversation(self, conversation_id):
        # check if the user is participant of the conversation, it helps fetch the conversation and
...
class MessageRetrieveDestroyView(generics.RetrieveDestroyAPIView):
    ...
    def perform_destroy(self, instance):
        if instance.sender != self.request.user:
            raise PermissionDenied("You are not a sender of this message")
        instance.delete()
